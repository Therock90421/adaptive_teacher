...
class ResNet(Backbone):
    ...
    def __init__(self, stem, stages, num_classes=None, out_features=None, freeze_at=0):
        ...
        ################## Adapt ResNet-101 for REACT ##################
        diff_in_channels = [256, 512, 1024]
        diff_out_channels = [256, 512, 1024]
        self.inv_attention = []
        self.spc = []
        self.spc_attention = []
        ...
        for i, (name, stage) in enumerate(zip(self.stage_names, self.stages)):
            for j in range(len(stage)):
                ...
                if j == 1:
                    diff_feature = x
            if i >= 0:
                inv_attention_layer = getattr(self, self.inv_attention[i])
                spc_layer = getattr(self, self.spc[i])
                spc_attention_layer = getattr(self, self.spc_attention[i])
                channel_attention_layer = getattr(self, self.channel_attention[i])
                spatial_attention_layer = getattr(self, self.spatial_attention[i])
